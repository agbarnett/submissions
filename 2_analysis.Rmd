...
```{r}
results_hours$pplot
jpeg('figures/hour_of_day.jpg', width=5, height=4, units='in', res=500, quality=100)
print(pplot)
invisible(dev.off())
#
...
#  geom_errorbar(width=0, col='dodgerblue')+ # use ribbon instead
  ...
  geom_text(data=text1, aes(x=x, y=y, ymin=ymin, ymax=ymax, label=label), size=2, adj=-0.1, nudge_y = 0.03)+
  geom_text(data=text2, aes(x=x, y=y, ymin=ymin, ymax=ymax, label=label), size=2, adj=-0.1, nudge_y = -0.03)+
  xlab('Hour of day')+
  ylab('Odds ratio')+
  g.theme
gplot
jpeg('figures/odds_ratios_hour.jpg', width=5, height=4, units='in', res=500, quality=100)
print(gplot)
invisible(dev.off())
```
