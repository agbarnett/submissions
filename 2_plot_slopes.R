...
# plotting the post-pandemic change in the probability of a weekend submission
# August 2022

# reference line for overall intercept and 95% CI
xref = filter(results_weekend$table, model==2, parameter=='change') %>%
  select(mean, lower, upper) %>%
  mutate_all( exp) # back transform mean and 95% CI

to.plot = filter(results_weekend$changes, model==2) %>%
...
export = FALSE
if(export==TRUE){
  jpeg('figures/slopes_weekend.jpg', width=5, height=4, units='in', res=400, quality=100)
  print(iplot)
  invisible(dev.off())
}